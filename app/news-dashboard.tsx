...
export default function Dashboard({endpoint="/api/news", snapshotMode=false}:{endpoint?:string;snapshotMode?:boolean}) {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><a href="./" className="brand"><img className="brand-logo" src="./cc-mama-logo.png" alt="三寶媽旅宿筆記 Logo" width={64} height={61}/><span>CC三寶媽-<br/>旅遊雷達<small>TRAVEL RADAR</small></span></a><div className="nav-caption">探索消息</div>
      ...
      <div className="sidebar-bottom"><Radio size={20}/><strong>保持掌握，輕鬆出發。</strong><p>{snapshotMode?"雲端每日早上 08:00（台灣時間）排程抓取；本頁每分鐘同步最新結果。":"頁面開啟時，每 24 小時檢查一次新消息。"}</p><Button variant="outline" onClick={()=>setSources(v=>!v)}>查看資料來源</Button></div><div className="personal-label">個人非商業閱讀版</div>
    </aside>
    <main><header className="topbar"><span>消息中心 <span className="divider">/</span> 最新動態</span><span className="topbar-note">日本 · 台灣 · 韓國</span></header><div className="workspace">
      ...
      <div className="status-bar" role="status"><span><span className={`status-dot ${error||staleSnapshot?"warning":""}`}/>{busy?"正在同步資料":data?`來源抓取時間 ${dateLabel(data.checkedAt)}（台北時間）`:"準備連接新聞來源"}</span><span>{snapshotMode?"每日 08:00 排程（台灣時間） · 網頁每 1 分鐘同步":"每 24 小時檢查"}</span></div>
      {lastReadAt&&<div className="sync-detail">本頁上次同步：{dateLabel(lastReadAt)}{snapshotMode&&<> · <a href="https://github.com/thxmama0601/travel-radar/actions/workflows/refresh-news.yml" target="_blank" rel="noopener noreferrer">查看雲端執行紀錄 ↗</a></>}</div>}{staleSnapshot&&<div className="error-box" role="alert">雲端資料已超過 26 小時未更新。重新整理只能同步已發布的資料；背景排程可能延遲或失敗，請查看雲端執行紀錄。</div>}{error&&<div className="error-box" role="alert">{error}</div>}{data&&data.sources.some(s=>s.status==="stale"||s.status==="error")&&!error&&<div className="error-box">部分來源暫時無法更新。<button onClick={()=>setSources(true)}>查看來源狀態</button></div>}
