...
export default function Dashboard({endpoint="/api/news", snapshotMode=false}:{endpoint?:string;snapshotMode?:boolean}) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const result:NewsPayload=await r.json(); setData(result); setNow(Date.now());
      if(result.sources.every(s=>s.status==="error")) setError("目前無法連接新聞來源，請稍後重試。");
    } catch { setError("無法取得最新消息，請檢查連線後重新整理。現有列表不代表已更新。"); }
    finally {setBusy(false);inFlight.current=false;}
  },[endpoint]);
  useEffect(()=>{void refresh();const timer=setInterval(()=>{setNow(Date.now());if(!document.hidden) void refresh();},REFRESH_INTERVAL_MS);const visible=()=>{if(!document.hidden) void refresh();};document.addEventListener("visibilitychange",visible);return()=>{clearInterval(timer);document.removeEventListener("visibilitychange",visible);};},[refresh]);
  ...
  return <div className="app-shell">
    <aside className="sidebar"><a href="./" className="brand"><span className="brand-icon"><Radar size={25}/></span><span>旅訊雷達<small>TRAVEL RADAR</small></span></a><div className="nav-caption">探索消息</div>
      ...
      <div className="sidebar-bottom"><Radio size={20}/><strong>保持掌握，輕鬆出發。</strong><p>{snapshotMode?"背景每 30 分鐘排程更新，關閉頁面也能持續追蹤。":"頁面開啟時，每 30 分鐘檢查一次新消息。"}</p><Button variant="outline" onClick={()=>setSources(v=>!v)}>查看資料來源</Button></div><div className="personal-label">個人非商業閱讀版</div>
    </aside>
    <main><header className="topbar"><span>消息中心 <span className="divider">/</span> 最新動態</span><span className="topbar-note">日本 · 台灣 · 韓國</span></header><div className="workspace">
      <div className="page-heading"><div><div className="eyebrow">YOUR NEXT TRIP, INFORMED</div><h1>最新旅遊動態<span className="heading-dot">.</span></h1><p>優惠、政策與交通消息，一站掌握。</p></div><Button className="refresh-button" variant="outline" onClick={()=>void refresh()} disabled={busy}><RefreshCw size={17} className={busy?"spin":""}/>{busy?"更新中":"重新整理"}</Button></div>
      <div className="status-bar" role="status"><span><span className={`status-dot ${error?"warning":""}`}/>{busy?"正在檢查新聞來源":data?`最近檢查 ${dateLabel(data.checkedAt)}（台北時間）`:"準備連接新聞來源"}</span><span>每 30 分鐘檢查 · 排程與收錄可能延遲</span></div>
      {snapshotMode&&data&&now-Date.parse(data.checkedAt)>90*60000&&<div className="error-box" role="status">資料已超過 90 分鐘未更新，背景排程可能延遲或失敗。請以原始來源為準。</div>}{error&&<div className="error-box" role="alert">{error}</div>}{data&&data.sources.some(s=>s.status==="stale"||s.status==="error")&&!error&&<div className="error-box">部分來源暫時無法更新。<button onClick={()=>setSources(true)}>查看來源狀態</button></div>}
